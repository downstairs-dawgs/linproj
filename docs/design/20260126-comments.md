...
interface CommentUpdateInput {
  ...
  resolvingCommentId?: string | null;  // Set to comment's own ID to resolve, null to unresolve
}
```
...
Comment resolution is handled through the `commentUpdate` mutation using the `resolvingCommentId` field. The `resolvedAt` timestamp is automatically set by Linear when resolved.
...
**To resolve:** Set `resolvingCommentId` to the comment's own ID (self-reference marks it resolved).

**To unresolve:** Set `resolvingCommentId` to `null`.

```typescript
// Resolve
await commentUpdate(commentId, { resolvingCommentId: commentId });

// Unresolve
await commentUpdate(commentId, { resolvingCommentId: null });
```
...
- [ ] Support stdin input
- [ ] Support editor mode (default when no argument/stdin and TTY)
- [ ] Support `--reply-to` for threading
- [ ] Add output options (`--json`, `--quiet`); default outputs URL
- [ ] Add E2E tests (`tests/e2e/issues-comments-add.test.ts`)
...
- [ ] Add E2E tests (`tests/e2e/issues-comment-edit.test.ts`)
...
- [ ] Add E2E tests (`tests/e2e/issues-comment-resolve.test.ts`)
...
- [ ] Add E2E tests (`tests/e2e/issues-get-comments.test.ts`)
