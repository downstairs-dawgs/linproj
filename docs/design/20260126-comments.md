...
linproj issues comment ENG-123 "Looking into this now"

# Multiline via stdin (for AI agents and scripting)
cat <<EOF | linproj issues comment ENG-123
...
linproj issues comment ENG-123 --edit
linproj issues comment ENG-123 -e

# Reply to a specific comment (threading)
linproj issues comment ENG-123 "Good idea!" --reply-to <comment-id>

# Reply to the last comment
linproj issues comment ENG-123 "Agreed" --reply-to last

# Output options (default: outputs comment URL)
linproj issues comment ENG-123 "Done" --json   # Return created comment as JSON
linproj issues comment ENG-123 "Done" --quiet  # Suppress output
```
...
All comment operations are subcommands of `issues`:

```
linproj issues comments <issue-id>          # List comments
linproj issues comment <issue-id> "message" # Add comment
linproj issues comment edit <id>            # Edit comment
linproj issues comment delete <id>          # Delete comment
linproj issues comment resolve <id>         # Resolve thread
linproj issues comment unresolve <id>       # Unresolve thread
```

This structure allows for future extension to other commentable objects (e.g., projects) while maintaining a consistent pattern.
...
```bash
linproj issues comment ENG-123 "Quick update: PR is ready for review"
```
...
```bash
echo "Automated comment from CI" | linproj issues comment ENG-123
...
Best for: AI agents, CI/CD pipelines, scripts. For file input, use `cat file.md | linproj issues comment ENG-123`.
...
> **Future consideration**: Investigate adding comments to `linproj issues get` output by default, with truncation for issues with extended comment discussion.
...
```
src/commands/issues/
├── comments.ts         # issues comments (list)
├── comment.ts          # issues comment (add)
├── comment-edit.ts     # issues comment edit
├── comment-delete.ts   # issues comment delete
└── comment-resolve.ts  # issues comment resolve/unresolve
```
...
describe('issues comment add', () => {
  test('adds comment via argument', async () => {
    const result = await runCLI(['issues', 'comment', 'TEST-123', 'Test comment']);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('linear.app'); // outputs URL by default
  });

  test('adds comment via stdin', async () => {
    const result = await runCLI(['issues', 'comment', 'TEST-123'], {
  ...
  test('adds reply to comment', async () => {
    const result = await runCLI([
      'issues', 'comment', 'TEST-123', 'Reply text',
  ...
  test('errors on issue not found', async () => {
    const result = await runCLI(['issues', 'comment', 'NONEXISTENT-999', 'test']);
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('not found');
  });
...
- [ ] Create `src/commands/issues/comment.ts`
...
- **Comments in `issues get`**: Show comments by default with truncation for long threads
